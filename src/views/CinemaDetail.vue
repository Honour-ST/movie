<template>
  <div v-if="cinema" class="cinema-detail">
    ...
    <div class="nav-showdate">
      <div class="datelist">
        <div
          ...
          v-for="(item,index) in nowShow.shows"
          :key="index"
        >{{item.dateShow}}</div>
      </div>
    </div>
    ...
    <iframe
      v-show="showmap"
      class="map"
      :src="`https://apis.map.qq.com/tools/poimarker?type=1&keyword=${cinema.nm}&region=${app.city}&radius=500&key=3IFBZ-YUJ6O-32FWX-SF5OE-ZJPSH-JTBLN&referer=myapp`"
      frameborder="0"
    ></iframe>
